...
class CM(object):
    ...
    def __init__(self):
        self.adb = 'adb'
        self.exceltime = time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime(time.time()))

    def get_cpu(self):
        try:
            cputext = subprocess.Popen(self.adb + ' shell dumpsys cpuinfo | grep video.like', shell=True, stdout=subprocess.PIPE,
    ...
    def get_memory(self):
        ...
        def cmd(i):
            try:
                ...
                pass
        memlist = []
        cputext = subprocess.Popen(self.adb + ' shell dumpsys meminfo video.like | grep -e Graphics -e Native -e Dalvik -e TOTAL',
    ...
    def get_battery(self):
        batterytext = subprocess.Popen(self.adb + ' shell dumpsys battery', shell=True, stdout=subprocess.PIPE,
    ...
    def matplo(self):
        ...
        while True:
            try:
                plt.clf()
                native = int(self.get_memory()[0])
                dalvik = int(self.get_memory()[1])
                total = int(self.get_memory()[2])
                graphics = int(self.get_memory()[3])
                ax.append(num)
                ax2.append(num)
                ax3.append(num)
                ax4.append(num)
                ...
                self.wb.save('{}.xlsx'.format(self.exceltime))
                num += 1
                a += 1
            except:
                print('error')
